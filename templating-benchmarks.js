...
templatesFiles.forEach(function (groupName) {
  ...
  var group =
    templateGroupsLookup[groupName] ||
    (templateGroupsLookup[groupName] = {
      name: groupName,
      templates: [],
      data: {}
  ...
  groupFilenames.forEach(function (filename) {
    if (
      filename.charAt(0) === '.' ||
      (!filename.startsWith('template') && !filename.startsWith('data'))
    ) {
      return
    }
    ...
    if (filename === 'data.js') {
      ...
      var outputFile = nodePath.join(
        baseHtmlOutputFile,
        engine.name + (variant ? '.' + variant : '') + '.html'
      )
      var outputCompileFile = nodePath.join(
        outputCompiledDir,
        groupName,
        engine.name + (variant ? '.' + variant : '') + '.js'
      )
      var outputCompileMinifiedFile = nodePath.join(
        outputCompiledMinifiedDir,
        groupName,
        engine.name + (variant ? '.' + variant : '') + '.min.js'
      )
...
function warmup (callback) {
    ...
    templateGroup.templates.forEach(function (templateInfo) {
      if (templateInfo.engine.load) {
        work.push(function (callback) {
          ...
          templateInfo.engine.load(
            src,
            templateInfo.templateFile,
            templateInfo.compileName,
        ...
        if (Array.isArray(data)) {
          data.forEach(function (data, i) {
            work.push(function (callback) {
              ...
              templateInfo.engine.render(template, data, function (
                err,
                output
              ) {
                if (err) {
          ...
          templateInfo.engine.compile(
            ...
            function (err, output) {
              ...
              try {
                ...
                throw new Error(
                  'Unable to minify "' +
                    templateInfo.outputCompileFile +
                    '". Exception: ' +
                    (e.stack || e)
                )
              }
              ...
              fs.writeFileSync(
                templateInfo.outputCompileMinifiedFile,
                minified,
                'utf8'
              )
              ...
              zlib.gzip(minified, function (err, gzippedBuffer) {
                ...
                sizeInfo.gzipped[templateInfo.description] =
                  gzippedBuffer.length
                sizeInfo.uncompressed[templateInfo.description] =
                  minifiedBuffer.length
                callback()
              })
            }
          )
  ...
  async.series(work, function (err, results) {
    exports.sizes = sizes
    fs.writeFileSync(
      nodePath.join(outputDir, 'sizes.json'),
