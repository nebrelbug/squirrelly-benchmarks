...
module.exports = function (runner, utils) {
  ...
  function reportSizes () {
    ...
    Object.keys(sizes).forEach(function (groupName) {
      ...
      Object.keys(gzippedSizes).forEach(function (template) {
        ...
        if (
          smallestUncompressed == null ||
          uncompressedSize < smallestUncompressed.uncompressedSize
        ) {
      ...
      for (var i = 0; i < sortedSizes.length; i++) {
        ...
        var percentLarger = (
          ((curSize - smallestSize) / curSize) *
          100
        ).toFixed(2)
        var percentLargerUncompressed = (
          ((uncompressedSize - smallestUncompressedSize) / uncompressedSize) *
          100
        ).toFixed(2)
        ...
        sizeLine(
          resultLine(
            ...
            color(
              padBefore(
                gzippedSizes[template] + ' bytes gzipped',
                leftColWidth
              ),
              'cyan'
            ) +
        ...
        if (sortedSizes[i] !== smallestUncompressed) {
          perRightCol = color(
            padBefore(percentLargerUncompressed, 5) + '% larger',
            'red'
          )
        } else {
        ...
        sizeLine(
          resultLine(
            '',
            padBefore(perLeftCol, leftColWidth) +
              '   ' +
              padBefore(perRightCol, rightColWidth),
            { separator: false }
          )
        )
      }
  ...
  runner.on('end', function (stats) {
    ...
    readmeMarkdown = require('./injector').inject(
      readmeMarkdown,
      resultsMarkdown
    )
  ...
  runner.on('suite end', function (suite) {
    ...
    if (benches.length) {
      ...
      for (i = 0; i < benches.length; i++) {
        ...
        if (i !== 0) {
          var percentSlower = (bench.percentSlower = (
            ((fastestOps - ops) / fastestOps) *
            100
          ).toFixed(2))
  ...
  runner.on('bench end', function (bench) {
    ...
    cursor
      ...
      .write(
        resultLine(
          bench.title,
          color(padBefore(ops, 9) + ' op/s', 'cyan') + '\n'
        )
      )
  })
}
